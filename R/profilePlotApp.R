...
profilePlotServer <- function(id, selected_dataset_category, trait_to_profile) {
 shiny::moduleServer(id, function(input, output, session) {
  ...
  current_data_paths <- reactive({
   ...
   # v3: resolve each data file via the data layer on first use.
   fst_key <- paste0("pheno_data_long_", sanitized_category, ".fst")
   rds_key <- paste0("trait_names_", sanitized_category, ".rds")
   list(
    fst = tryCatch(local_path(fst_key, must_exist = FALSE), error = function(e) NA_character_),
    rds = tryCatch(local_path(rds_key, must_exist = FALSE), error = function(e) NA_character_)
   )
  })

  # Now directly uses the trait passed from the main app
  selected_trait_data <- reactive({
   req(trait_to_profile(), nzchar(trait_to_profile()))
   paths <- current_data_paths()

   req(!is.na(paths$fst), file.exists(paths$fst))
   ...
   if (!is.null(category) && category == "Liver Splice Junctions") {
    tryCatch(
     {
      annot_path <- tryCatch(local_path("annotation_list.rds", must_exist = FALSE), error = function(e) NA_character_)
      if (is.na(annot_path) || !file.exists(annot_path)) stop("annotation_list.rds unavailable")
      annot_list <- readRDS(annot_path)
  ...
  # Whether we have exactly one grouping variable
  ...
  output$profile_boxplot <- plotly::renderPlotly({
   ...
   # Prepare numeric positions for categorical x. (x_jitter no longer used —
   # we removed the jittered marker layer in favor of the violin showing
   ...
   # ---- Palette + plot-mode resolution ----
   # When grouping is exactly Sex+Diet we render as a split violin: Sex
   # becomes the X axis, and each violin is split into two halves (HC left,
   # HF right). Override tick_vals / tick_text for that case so the X axis
   # shows just Sex; the underlying data still carries Sex AND Diet so the
   # split traces below can pull each combination.
   sex_colors  <- c("F" = "#B2182B", "M" = "#D6604D")
   diet_colors <- c("HC" = "#92C5DE", "HF" = "#2166AC")
   both_colors <- c("F:HC" = "#7BC17E", "F:HF" = "#E8943A",
                    "M:HC" = "#5B8FD4", "M:HF" = "#B07BC1")

   profile_mode <- if (length(grouping_vars) == 2L && setequal(grouping_vars, c("Sex", "Diet"))) {
    "split_sex_diet"
   } else if (length(grouping_vars) == 1L && identical(grouping_vars, "Sex")) {
    "by_sex"
   } else if (length(grouping_vars) == 1L && identical(grouping_vars, "Diet")) {
    "by_diet"
   } else {
    "neutral"
   }

   if (identical(profile_mode, "split_sex_diet")) {
    sex_levels <- intersect(c("F", "M"), levels(plot_data_clean$Sex))
    tick_text <- sex_levels
    tick_vals <- seq_along(sex_levels)
    plot_data_clean[, x_index := match(as.character(Sex), sex_levels)]
   }
   ...
   # ---- Distribution layer: violin + embedded box ----
   # No jittered points: the violin already shows the distribution shape and
   # the embedded box shows quartiles. The plot is colored via group-aware
   # palettes (sex_colors / diet_colors / both_colors) defined above. For
   # the Sex+Diet combined case we render a split violin (HC half-left,
   # HF half-right) so each Sex position visually separates the two diets.
   p <- plotly::plot_ly()

   # Shared violin styling — shape, box, mean line. Per-trace fill/line
   # color is filled in below depending on the mode.
   violin_box <- list(
    visible = TRUE,
    fillcolor = "rgba(255, 255, 255, 0.92)",
    line = list(color = "#2c3e50", width = 1)
   )
   violin_meanline <- list(visible = TRUE, color = "#2c3e50", width = 1.5)

   # Drop alpha into a hex fillcolor — plotly accepts rgba() strings reliably.
   ...
   if (identical(profile_mode, "split_sex_diet")) {
    # One trace per (Sex, Diet) combo. side="negative" puts HC on the left
    # half of each Sex position; side="positive" puts HF on the right. The
    # `scalegroup` ties paired halves so both use the same scale (otherwise
    # one side could dominate visually if one diet has wider distribution).
    for (sex_lvl in tick_text) {
     for (diet_lvl in c("HC", "HF")) {
      sub <- plot_data_clean[Sex == sex_lvl & Diet == diet_lvl]
      if (nrow(sub) == 0L) next
      key <- paste0(sex_lvl, ":", diet_lvl)
      side <- if (identical(diet_lvl, "HC")) "negative" else "positive"
      fill_hex <- both_colors[[key]] %||% "#2c3e50"
      p <- plotly::add_trace(
       p,
       data = sub,
       x = ~x_index,
       ...
       side = side,
       scalegroup = sex_lvl,
       width = 1.0,            # span the full categorical slot — halves meet at center, adjacent positions touch
       points = FALSE,
       spanmode = "soft",
       scalemode = "width",
       line = list(color = "#2c3e50", width = 1.1),
       fillcolor = hex_to_rgba(fill_hex, 0.75),
       box = violin_box,
       meanline = violin_meanline,
       hoveron = "violins",
       name = key,
       legendgroup = diet_lvl,
       showlegend = identical(sex_lvl, tick_text[1L])  # one legend entry per diet
      )
     }
    }

   } else if (identical(profile_mode, "by_sex")) {
    # One violin per Sex level, fill color from sex_colors.
    for (sex_lvl in intersect(c("F", "M"), levels(plot_data_clean$Sex))) {
     sub <- plot_data_clean[Sex == sex_lvl]
     if (nrow(sub) == 0L) next
     fill_hex <- sex_colors[[sex_lvl]] %||% "#2c3e50"
     p <- plotly::add_trace(
      p,
      data = sub,
      x = ~x_index,
      y = ~Value,
      type = "violin",
      width = 1.0,            # fill the full categorical slot — adjacent violins touch instead of leaving gaps
      points = FALSE,
      spanmode = "soft",
      scalemode = "width",
      line = list(color = "#2c3e50", width = 1.1),
   ...
   } else if (identical(profile_mode, "by_diet")) {
    for (diet_lvl in intersect(c("HC", "HF"), levels(plot_data_clean$Diet))) {
     sub <- plot_data_clean[Diet == diet_lvl]
     if (nrow(sub) == 0L) next
     fill_hex <- diet_colors[[diet_lvl]] %||% "#2c3e50"
     p <- plotly::add_trace(
      p,
      data = sub,
      x = ~x_index,
    ...
    # Neutral fallback (Generation, Litter, GenLit, multi-grouping with
    # Generation/Litter, etc.) — single trace, soft accent-blue fill.
    p <- plotly::add_trace(
     p,
   ...
   # Always reserve y-axis room for the p-value annotations + a small
   # buffer above and below the data, regardless of where annotations end
   # up. The previous conditional only extended the range when annotations
   # dropped below y_min and could still leave bottom annotations clipped.
   y_low_view  <- y_min
   y_high_view <- y_max
   if (isTRUE(input$show_p_annotations) && isTRUE(single_grouping_selected()) &&
       length(annotations) > 0L) {
    ann_y_vec <- vapply(annotations, function(a) a$y, numeric(1))
    ann_y_vec <- ann_y_vec[is.finite(ann_y_vec)]
    if (length(ann_y_vec) > 0L) {
     y_low_view  <- min(y_low_view,  min(ann_y_vec))
     y_high_view <- max(y_high_view, max(ann_y_vec))
    }
   }
   y_view_pad <- max(0.08 * (y_high_view - y_low_view), 0.5 * y_margin)
   y_low_view  <- y_low_view  - y_view_pad
   y_high_view <- y_high_view + y_view_pad

   # Layout: blank background, full axis lines on all four sides ("boxed"
   # plot) for a cleaner, more publication-ready look. Light horizontal
   # gridlines kept on the y-axis for value reading; vertical gridlines
   # dropped since they'd compete with the violin shapes.
   p <- plotly::layout(
    p,
    title = list(
    ...
    xaxis = list(
     ...
     linecolor = "#2c3e50",
     linewidth = 1,
     mirror = TRUE,           # draw axis line on top edge too — boxes the plot
     showgrid = FALSE,
     zeroline = FALSE,
     ticks = "outside",
     tickcolor = "#2c3e50"
    ...
    yaxis = list(
     title = "<b>Phenotype Value</b>",
     range = c(y_low_view, y_high_view),   # always include p-value annotation room
     showline = TRUE,
     linecolor = "#2c3e50",
     linewidth = 1,
     mirror = TRUE,           # draw axis line on right edge too
     showgrid = TRUE,
     gridcolor = "#eef1f5",
     zeroline = FALSE,
     ticks = "outside",
    ...
    violinmode = "overlay"     # split halves overlay at same x — required for split mode
   )
