...
mediation_tab_ui <- function(current_category = NULL) {
 ...
 # Discover mediation files via the active backend.
 #
 # IMPORTANT: on backend=s3 we cannot afford an s3_list at the bucket root —
 # it paginates ~850k objects to filter for the ~20 mediation CSVs and takes
 # ~3 minutes per session, blowing past shiny-server's app_init_timeout.
 # Mediation file names follow a strict convention so we generate the full
 # 20-file inventory from first principles: each combination of
 #   first   ∈ {liver_genes, liver_isoforms}
 #   variant ∈ {all_mice_additive, qtlxdiet_in_HC_mice_additive,
 #              qtlxdiet_in_HF_mice_additive, qtlxsex_in_female_mice_additive,
 #              qtlxsex_in_male_mice_additive}
 #   second  ∈ {liver_genes, liver_isoforms}
 # forming "<first>_<variant>_<second>_mediator_all_mediators.csv".
 # Downstream code resolves each rel key via local_path() at read time;
 # any candidate that doesn't actually exist on S3 will error out then,
 # which is surfaced to the user — far better than blocking app boot for
 # 3 minutes on every session.
 mediation_candidate_files <- function() {
  firsts   <- c("liver_genes", "liver_isoforms")
  variants <- c(
   "all_mice_additive",
   "qtlxdiet_in_HC_mice_additive", "qtlxdiet_in_HF_mice_additive",
   "qtlxsex_in_female_mice_additive", "qtlxsex_in_male_mice_additive"
  )
  seconds <- c("liver_genes", "liver_isoforms")
  grid <- expand.grid(first = firsts, variant = variants, second = seconds,
                      stringsAsFactors = FALSE)
  paste0(grid$first, "_", grid$variant, "_", grid$second, "_mediator_all_mediators.csv")
 }

 files <- tryCatch({
  if (identical(qtl_config()$backend, "s3")) {
   mediation_candidate_files()
  } else {
   root <- qtl_config()$data_root
   if (dir.exists(root)) {
    list.files(root, pattern = "_mediator_all_mediators\\.csv$", full.names = TRUE, recursive = TRUE)
   } else {
    character(0)
   }
  }
