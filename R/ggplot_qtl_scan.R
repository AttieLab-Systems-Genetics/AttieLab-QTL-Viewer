...
ggplot_qtl_scan <- function(scan_table, LOD_thr = NULL, selected_chr = "All",
 ...
 color_map <- c(
  "Additive" = "#7F7F7F", # Grey (matches Distal in cis-trans plot; overridden to green when top peak is local — see additive_color_override below)
 ...
 # Caller may override the Additive color to reflect peak locality:
 #   "#69B369" (green) if the trait's top peak is local (cis)
 #   "#7F7F7F" (grey) if the top peak is distal (trans)
 # Matches the cis-trans plot palette so users see the same Local/Distal
 # signal in both views. Only applies to the Additive trace; the
 # interactive overlays keep their own colors.
 if (!is.null(additive_color_override) && nzchar(additive_color_override)) {
  color_map["Additive"] <- additive_color_override
 }
