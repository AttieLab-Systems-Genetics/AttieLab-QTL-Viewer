...
#
# Performance: every column of `pheno_sub` shares the same design matrix
# (~ GenLit + Sex), so the original implementation's per-column lm() loop
# was dominated by formula-parsing / model.matrix / na.action overhead —
# ~80s for 85k isoform columns. We split the work in two:
#   - Columns with no NAs in y: residualize the full block in ONE call to
#     qr.resid(QR, Y), which does a single matrix multiply per column
#     (effectively O(n * p * T) total instead of T * O(n * p^2 + parsing)).
#   - Columns with NAs: each column has a different "kept rows" set, so
#     fall back to per-column .lm.fit (the bare-bones variant — no
#     formula, no copies). Still much faster than stats::lm.
ensure_adjusted_matrix <- function(target_dataset = "liver_isoforms") {
 ...
 # Build the shared design matrix once. Drop rows where any covariate is NA
 # (defensive — covariates should be complete, but if not we propagate NAs
 # cleanly for those rows in the output rather than failing the whole call).
 X_full <- stats::model.matrix(~ GenLit + Sex)
 covar_complete <- stats::complete.cases(GenLit, Sex)
 if (!all(covar_complete)) {
  warning("ensure_adjusted_matrix: ", sum(!covar_complete),
   " row(s) have NA covariates and will be NA in the adjusted matrix")
 }
 X <- X_full[covar_complete, , drop = FALSE]
 pheno_complete <- pheno_sub[covar_complete, , drop = FALSE]

 pheno_adj <- pheno_sub                # preserves dimnames, NA rows for incomplete covars
 pheno_adj[!covar_complete, ] <- NA_real_
 pheno_adj[covar_complete, ] <- NA_real_   # will be filled in below

 # Split columns: fully observed in pheno_complete vs partial-NA. The
 # all-NA / constant-y columns we leave as NA (matches old behavior of
 # skipping those columns).
 col_n_obs <- colSums(!is.na(pheno_complete))
 fully_obs_cols <- which(col_n_obs == nrow(pheno_complete))
 partial_cols   <- which(col_n_obs > 0L & col_n_obs < nrow(pheno_complete))

 # Sanity: drop columns whose non-NA values are constant (qr.resid on a
 # constant column would just return zeros, which is fine — but for parity
 # with the old loop's "skip constant columns" branch we leave them as NA).
 # Vectorized variance-detection in base R: var = (sum_sq - n * mean^2) / (n - 1)
 if (length(fully_obs_cols) > 0L) {
  Y_check <- pheno_complete[, fully_obs_cols, drop = FALSE]
  n_rows <- nrow(Y_check)
  col_means <- colMeans(Y_check)
  col_var <- (colSums(Y_check * Y_check) - n_rows * col_means^2) / max(n_rows - 1L, 1L)
  variance_ok <- col_var > .Machine$double.eps
  fully_obs_cols <- fully_obs_cols[variance_ok]
 }

 # ---- batch residualize fully-observed columns via shared QR ----
 if (length(fully_obs_cols) > 0L) {
  t0 <- Sys.time()
  qr_X <- qr(X)
  Y_block <- pheno_complete[, fully_obs_cols, drop = FALSE]
  # NB: qr.resid is in base, not stats — earlier `stats::qr.resid` raised
  # "is not an exported object from 'namespace:stats'" and prewarm fell
  # through to the lazy-on-click path (~80s on first user click).
  resid_block <- qr.resid(qr_X, Y_block)
  pheno_adj[covar_complete, fully_obs_cols] <- resid_block
  message(sprintf("  batch residualized %d fully-observed columns in %.2fs",
   length(fully_obs_cols),
   as.numeric(difftime(Sys.time(), t0, units = "secs"))))
 }

 # ---- per-column for columns with NAs (different kept-row set each) ----
 if (length(partial_cols) > 0L) {
  t0 <- Sys.time()
  for (j in partial_cols) {
   y <- pheno_complete[, j]
   ok <- !is.na(y)
   if (sum(ok) <= ncol(X) || length(unique(y[ok])) <= 1L) next
   Xs <- X[ok, , drop = FALSE]
   ys <- y[ok]
   fit <- .lm.fit(Xs, ys)
   resid_col <- rep(NA_real_, length(y))
   resid_col[ok] <- fit$residuals
   pheno_adj[covar_complete, j] <- resid_col
  }
  message(sprintf("  per-column residualized %d partial-NA columns in %.2fs",
   length(partial_cols),
   as.numeric(difftime(Sys.time(), t0, units = "secs"))))
 }

 message(sprintf("ensure_adjusted_matrix: done in %.2fs",
