...
correlationServer <- function(id, import_reactives, main_par) {
 shiny::moduleServer(id, function(input, output, session) {
  ...
  # Per-column in-memory cache for correlation files. The pre-fix path read
  # 2-3 columns from a >100 MB FST on EVERY trait click \u2014 a different
  # `key_info$key` column each time. That hit the disk repeatedly for the
  # same file, paying the 30-60s decompression cost on every click. With
  # this cache, each (file, mtime, column) combination is read at most
  # once per process; the constant "phenotype"/"Phenotype" columns are read
  # on the first click and reused forever, and trait columns are reused
  # whenever the same trait is revisited. Cache lives in the module scope
  # (per Shiny session); it's cleared automatically when the session ends.
  .corr_col_cache <- new.env(parent = emptyenv())

  # Read a correlation/pval/nmice file efficiently. Accepts a rel key; resolves
  # to local via the S3 cache first. Memoized at the column level \u2014 see above.
  read_corr_file <- function(file_key, select_cols = NULL) {
   ...
   # Bypass cache when the caller wants the whole file (select_cols = NULL).
   # That path is rare and usually means a small file; not worth caching.
   if (is.null(select_cols)) {
    if (grepl("\\.fst$", local, ignore.case = TRUE) &&
        requireNamespace("fst", quietly = TRUE)) {
     return(data.table::as.data.table(fst::read_fst(local)))
    }
    return(data.table::fread(local, showProgress = FALSE))
   }

   # mtime-stamp the cache key so any file replacement invalidates entries.
   mtime <- as.numeric(file.info(local)$mtime)
   col_key <- function(col) paste0(local, "|", mtime, "|", col)

   # Phase 1: serve cached columns; collect misses.
   collected <- list()
   misses <- character(0)
   for (col in select_cols) {
    cached <- get0(col_key(col), envir = .corr_col_cache, inherits = FALSE)
    if (!is.null(cached)) {
     collected[[col]] <- cached
    } else {
     misses <- c(misses, col)
    }
   }

   # Phase 2: read missing columns from disk in one I/O, populate cache.
   if (length(misses) > 0L) {
    if (grepl("\\.fst$", local, ignore.case = TRUE)) {
     if (!requireNamespace("fst", quietly = TRUE)) {
      warning("fst package not available; falling back to CSV")
      csv_fallback <- corr_resolve(sub("\\.fst$", ".csv", file_key))
      fresh <- data.table::as.data.table(data.table::fread(csv_fallback, showProgress = FALSE))
     } else {
      cols_in_file <- fst::metadata_fst(local)$columnNames
      to_read <- intersect(misses, cols_in_file)
      if (length(to_read) > 0L) {
       fresh <- data.table::as.data.table(fst::read_fst(local, columns = to_read))
      } else {
       fresh <- data.table::data.table()
      }
     }
    } else {
     hdr <- names(data.table::fread(local, nrows = 0))
     to_read <- intersect(misses, hdr)
     fresh <- if (length(to_read) > 0L) {
      data.table::fread(local, select = to_read, showProgress = FALSE)
     } else {
    ...
    for (col in names(fresh)) {
     v <- fresh[[col]]
     assign(col_key(col), v, envir = .corr_col_cache)
     collected[[col]] <- v
    }
   }

   # Preserve caller's column order; drop any that genuinely don't exist.
   collected <- collected[intersect(select_cols, names(collected))]
   if (length(collected) == 0L) return(data.table::data.table())
   data.table::as.data.table(collected)
  }
  ...
  list_available_pairs <- shiny::reactive({
   ...
   files <- character(0)
   # List correlation files via the active backend.
   #
   # IMPORTANT: when correlations_prefix is empty ("") AND backend is S3,
   # the s3_list call would scan the entire bucket prefix (currently
   # ~850k keys) just to filter for ~30 _corr.(csv|fst) names \u2014 that
   # takes ~200s on the campus.s3.wisc.edu endpoint and was causing
   # the worker to be killed by shiny-server's app_init_timeout on
   # every fresh session. The hardcoded `expected_files` list below
   # is the canonical inventory of correlation files anyway, so we
   # skip the discovery scan in that case and let the merge step
   # populate `files` from `expected_files` alone.
   all_files <- tryCatch({
    if (identical(qtl_config()$backend, "s3")) {
     if (!nzchar(correlations_prefix)) {
      message("correlationServer: skipping root-level s3_list (correlations_prefix=''); using expected_files inventory.")
      character(0)
     } else {
