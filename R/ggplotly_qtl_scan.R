...
ggplotly_qtl_scan <- function(scan_object, peak_table,
 ...
 # Convert ggplot to plotly. We deliberately omit `width`/`height` so the
 # plotly object stays responsive — its on-screen size is governed by the
 # plotlyOutput container (width = "100%"), matching the split-by overlay
 # plot behavior. The plot_width / plot_height args are still honored as
 # the export resolution by the PNG/PDF download path elsewhere.
