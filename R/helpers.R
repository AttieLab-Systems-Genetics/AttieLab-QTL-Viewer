...
get_trait_choices <- function(import_data, selected_dataset = NULL) {
 ...
 if (trait_type %in% c("genes", "isoforms")) {
  ...
  base_choices <- unique(trait_list_df$symbol)
  base_choices <- sort(unique(base_choices[!is.na(base_choices) & nzchar(base_choices)]))

  # If the per-trait manifest for this dataset has multiple gene_id rows for
  # the same symbol (e.g. Gcat → two distinct Ensembl IDs), expand those into
  # one entry per gene_id so users see both candidates. Bare ambiguous
  # symbols are removed in favour of the explicit forms.
  manifest <- tryCatch(get_trait_manifest_for_group(selected_dataset),
                       error = function(e) NULL)
  if (!is.null(manifest) &&
      all(c("trait", "gene_id") %in% names(manifest))) {
   gid_ok <- !is.na(manifest$gene_id) & nzchar(manifest$gene_id)
   sym_counts <- table(manifest$trait[gid_ok])
   ambig_syms <- names(sym_counts)[sym_counts > 1L]
   if (length(ambig_syms) > 0L) {
    keep <- !(base_choices %in% ambig_syms)
    kept <- base_choices[keep]
    expansions <- character(0)
    for (s in ambig_syms) {
     m <- manifest[manifest$trait == s & gid_ok, , drop = FALSE]
     m <- m[order(as.character(m$gene_id)), , drop = FALSE]
     labels <- paste0(s, " (", m$gene_id, ")")
     values <- paste0(s, "|", m$gene_id)
     expansions <- c(expansions, stats::setNames(values, labels))
    }
    merged_values <- c(kept, unname(expansions))
    merged_labels <- c(kept, names(expansions))
    ord <- order(merged_labels)
    return(stats::setNames(merged_values[ord], merged_labels[ord]))
   }
  }
...
#' Split a picker value into (trait, gene_id).
#'
#' The picker emits ambiguous gene-symbol selections as "Symbol|gene_id"
#' (e.g. "Gcat|ENSMUSG00000006378"). Unambiguous selections are bare symbols.
#' Returns a list with `trait` (always) and `gene_id` (NULL when absent).
#' @export
split_trait_picker_value <- function(value) {
 if (is.null(value) || !nzchar(value)) {
  return(list(trait = value, gene_id = NULL))
 }
 parts <- strsplit(as.character(value), "|", fixed = TRUE)[[1]]
 trait <- parts[1]
 gene_id <- if (length(parts) >= 2L && nzchar(parts[2])) parts[2] else NULL
 list(trait = trait, gene_id = gene_id)
}

#' Resolve a bare ambiguous gene symbol to its first per-gene_id picker value.
#'
#' Used by the cis-trans-plot click handler so that clicking a point with an
#' ambiguous symbol (e.g. "Gcat") sets the trait-search dropdown to a value
#' the dropdown actually contains (e.g. "Gcat|ENSMUSG00000006378") rather
#' than leaving the box empty. Falls through to `symbol` unchanged when the
#' manifest is unavailable, the symbol isn't ambiguous, or anything errors.
#' @export
resolve_ambiguous_symbol_to_first_value <- function(group_label, symbol) {
 if (is.null(symbol) || !nzchar(symbol)) return(symbol)
 manifest <- tryCatch(get_trait_manifest_for_group(group_label),
                      error = function(e) NULL)
 if (is.null(manifest) || !all(c("trait", "gene_id") %in% names(manifest))) {
  return(symbol)
 }
 m <- manifest[manifest$trait == symbol &
               !is.na(manifest$gene_id) &
               nzchar(manifest$gene_id), , drop = FALSE]
 if (nrow(m) <= 1L) return(symbol)
 m <- m[order(as.character(m$gene_id)), , drop = FALSE]
 paste0(symbol, "|", m$gene_id[1])
}
