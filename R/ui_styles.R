...
custom_css <- shiny::HTML("
 ...
 /* Accent palette retuned from bright blue (formerly #3498db / #2980b9)
    to a neutral grey scale. All var(--primary-blue) / var(--secondary-blue)
    references downstream pick up the new hexes automatically. Names kept
    as-is to avoid a wide rename; treat them as primary/secondary accent. */
 :root {
  --primary-blue: #6c757d;
  --secondary-blue: #495057;
...
create_title_panel <- function(title, subtitle = NULL, icon = "\U0001F9EC") {
 div(
...
create_gradient_header <- function(title, subtitle = NULL, icon = "\U0001F50D") {
 div(
...
create_genomics_section <- function(..., title = NULL, icon = "\U0001F4CA") {
...
create_plot_container <- function(..., style = NULL, icon = "\U0001F4C8") {
 div(
...
create_datatable_container <- function(..., style = NULL, title = NULL, icon = "\U0001F4CB") {
 content <- if (!is.null(title)) {
  tagList(
...
create_genomics_select <- function(inputId, label, choices, selected = NULL, icon = "\U0001F53D", ...) {
...
create_trait_search_input <- function(inputId, label = NULL, placeholder = "Search traits/genes...", icon = "\U0001F50D", ...) {
...
create_genomics_slider <- function(inputId, label, min, max, value, step = NULL, icon = "\U0001F39A\uFE0F", ...) {
...
create_download_button <- function(outputId, label, type = "primary", icon = "\U0001F4BE", ...) {
 class_name <- switch(type,
  "primary" = "btn-genomics-primary",
  "secondary" = "btn-genomics-secondary",
  "btn-genomics-primary"
...
create_plot_output <- function(outputId, width = "100%", height = "auto",
                spinner_type = 8, spinner_color = "#6c757d",
                title = NULL, icon = "\U0001F4CA") {
...
create_datatable_output <- function(outputId, title = NULL, icon = "\U0001F4CB", compact = TRUE) {
...
create_section_with_subtitle <- function(title, subtitle, ..., icon = "\U0001F9EC") {
 tagList(
...
create_genomics_card <- function(header, body, footer = NULL, icon = "\U0001F52C") {
 bslib::card(
  class = "fade-in",
  bslib::card_header(
   class = "gradient-header",
  ...
  tagList(
   ...
   div(
    ...
    create_genomics_button(
     button_id,
     "\U0001F680 Search & Plot LOD Scan",
     type = "primary",
     style = "width: 100%;"
    )
   )
